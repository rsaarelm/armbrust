//! Hardware bindings for the exact single board I have on my desk.
...
impl UsartLayout {
    pub fn send(&mut self, c: u16) {
        unsafe {
            loop {
                if volatile_load(&self.isr) & (1 << 7) != 0 {
                    break;
                }
            }
        }
...
pub const USART1: *mut UsartLayout = 0x4001_3800 as *mut UsartLayout;
pub const USART2: *mut UsartLayout = 0x4000_4400 as *mut UsartLayout;
